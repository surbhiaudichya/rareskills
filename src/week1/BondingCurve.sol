...
contract BondingCurve is ERC20 {
    // Define events
    event TokenPurchased(address buyer, uint amount, uint totalPrice);
    event TokenSold(address seller, uint amount, uint totalPrice);

    // Define variables
    uint public initialPrice;
    uint public priceChangeRate;
    uint256 public reserveBalance;
    mapping(address => uint256) public lastBuyTimestamp;
    uint256 public constant cooldownPeriod = 1 days;

    constructor(
        uint256 _priceChangeRate,
        uint256 _initialPrice
    ) ERC20("Bonding Curve Continuous Token", "BCCT") {
     ...
     */
    function sellTokens(uint depositAmount) external {
        if (depositAmount == 0) {
            revert ZeroAmount();
        }
        ...
        if (block.timestamp < lastBuyTimestamp[msg.sender] + cooldownPeriod) {
            revert CooldownNotElapsed(
                block.timestamp - lastBuyTimestamp[msg.sender] + cooldownPeriod
            );
        }
     ...
     */
    function getCostToBuy(
        uint256 purchaseAmount
    ) public view returns (uint256 totalCost) {
        uint256 currentTotalSupply = totalSupply();
        uint256 reserveBeforeTrade = (priceChangeRate *
            currentTotalSupply ** 2) /
            2e36 +
            (initialPrice * currentTotalSupply) /
            1e18;
        uint256 updatedTotalSupply = currentTotalSupply + purchaseAmount;
        uint256 reserveAfterTrade = (priceChangeRate *
            updatedTotalSupply ** 2) /
            2e36 +
            (initialPrice * updatedTotalSupply) /
            1e18;
